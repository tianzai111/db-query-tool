...
export const Home: React.FC = () => {
  ...
  // AI assistant: whether to show the proactive "export this result?" prompt.
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      ...
      // Proactively nudge the user to export when there is data.
  ...
  /**
   * Unified export handler used by both the result-card toolbar and the
   * proactive AI prompt. It exports the currently loaded result in the
   * browser (instant), and offers a large-dataset confirmation.
   */
  const handleExport = (format: ExportFormat) => {
    ...
    const doExport = () => {
      if (!selectedDatabase || !queryResult) return;
      try {
        exportResultClient(queryResult, format, selectedDatabase);
  ...
  /**
   * One-click "run query + export" that talks directly to the backend export
   * endpoint. This is what an AI Agent / custom command would call to
   * complete both steps in a single round trip.
   */
  const handleQuickExport = async (format: ExportFormat) => {
    ...
    setExporting(true);
    try {
      const { filename, bytes } = await exportResultServer(
        selectedDatabase,
        sql.trim(),
        format
      );
      message.success(`Exported ${filename} (${(bytes / 1024).toFixed(1)} KB)`);
    } catch (error: any) {
